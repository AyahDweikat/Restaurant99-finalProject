...
import { emailTemplate } from "../../../Services/htmlForEmail.js";
...
export const newConfirmEmail = async (req, res) => {
  ...
  token = generateToken({ email }, process.env.SIGNUP_TOKEN, 60 * 60 * 12); // 12 hours
  const link = `${req.protocol}://${req.headers.host}/auth/confirmEmail/${token}`;
...
export const login = async (req, res, next) => {
  ...
  const refreshToken = generateToken({ id: user._id, role:user.role }, process.env.LOGIN_TOKEN, 60*60*24*30 );
...
export const forgetPassword = async(req, res, next)=>{
  ...
  const user = await userModel.findOne({ email });
  if (!user) return next(new Error("Not Registered Email", {cause:404}));
