...
export const deleteCategory = async (req, res, next) => {
  const category = await categoryModel.findById(req.params.categoryId);
  if (!category) return next(new Error(`Invalid Category Id`, { cause: 400 }));
  const deletedItems = await menuItemModel.find({
    categoryId: req.params.categoryId,
  });
  deletedItems.forEach(async (item) => {
    await menuItemModel.findByIdAndDelete(item._id);
  });
  const deletedProcess = await categoryModel.findByIdAndDelete(
    req.params.categoryId
  );
  return res
    .status(200)
    .json({ message: "Category and Products connected with it are deleted successfully", deletedProcess });
};
...
export const getAllCategories = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "get all Categories", results: categories });
};
